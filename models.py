...
class GRUD:
    def __init__(self, num_units, num_layers, input_means training_keep_prob,
                 bptt_length, batch_size, n_outputs):
        self.training_keep_prob = training_keep_prob
        self.keep_prob = tf.placeholder(tf.float32, shape=[])
        self.inputs = tf.placeholder(tf.int32, shape=[None, bptt_length],
                                    name="inputs")
        ...
        one_cell = lambda: LayerNormBasicGRUDCell(num_units, input_means,
        ...
        with tf.variable_scope("softmax_layer"):
            ...
            self.b = tf.get_variable("b", dtype=tf.float32,
                                     ...
                                     trainable=True)
            logits = self.W * last_outputs + self.b
